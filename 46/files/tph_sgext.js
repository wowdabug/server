...
function sg_kapow_showscore() {
	try {
		boards.displayScoreboard({
		  'metric': 'score',
		  'interval': 'daily'
		})
	} catch(e) {}
...
function sg_kapow_share(type,txt) {
	var sharetype = null;
	if (type == 1) {
		sharetype = "facebook";
	}
	try {
		social.share(txt, medium(sharetype),
		function() {
			gml_Script_gmcallback_kapowshareok();
			alert("share okay");
		}, 
		function() {
			gml_Script_gmcallback_kapowerror();
			alert("share failed");
		});
	} catch(e) {}
}

function sg_kapow_save(key,value) {
	try {
...
function sg_kapow_anal(eventName,k1,v1,k2,v2,k3,v3,k4,v4,k5,v5,k6,v6,k7,v7) {
	try {
		var obj = {};
		if (k1 != "") {
			obj[k1] = v1;
		...
		analytics.sendEvent(eventName, obj);
		alert(obj)
	} catch(e) {}
}

function sg_kapow_close() {
	try {
		close();
	} catch(e) {}
}
...
function sg_lag_ad(isreward) {
	try {
		if(typeof prerollStart === 'undefined'){
			...
			LaggedAPI.APIAds.show('interstitial','snake-vs-icy-bricks','snake-vs-icy-bricks-game.jpg',function(response) {
			if(response.success) {
				console.log('ad done');
				if (isreward) {
...
document.addEventListener("adBreakStart", () => {
	...
	gml_Script_gmcallback_cooladstart();
});  
document.addEventListener("adBreakComplete", () => {
	console.log("adBreak Complete")
	gml_Script_gmcallback_cooladend();
}); 

function sg_coolmath_ad() {
};
