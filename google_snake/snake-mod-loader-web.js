...
let externalConfig = {
  ...
  load: function() {
    ...
    req.open('GET', '../../other/mod-info.json', false);
    req.onload = function() {
      if(this.status === 200) {
        try {
...
document.body.appendChild = function(el) {
  ...
  if(el.src) {
	if(el.src && el.src === '' || el.src.includes('apis.google.com') || !el.src.includes('www.google')) return document.body.appendChildOld(el);
  }
  ...
  let isSrcCorrectFormat = false;
  if(el.src) {
	isSrcCorrectFormat = el.src.includes('funbox') || el.src.includes('xjs=s1') || (!window.location.href.includes('fbx?fbx=snake_arcade') && el.src.includes('/xjs/'));
	isSrcCorrectFormat = isSrcCorrectFormat || (WEB_VERSION && el.src.includes('snake.js'));
  }
  
   //As we apply url re-writes early on, we let snake.js in the web version through here
  ...
  */
  ...
  <div id="mod-selector-dialogue" style="display: block;margin:25px auto;padding:10px;border: 1px solid var(--mod-loader-thin-border);width:550px;background-color: var(--mod-loader-main-bg) !important;border-radius:5px;-webkit-box-shadow: 0px 0px 10px 1px rgba(0,0,0,0.24);box-shadow: 0px 0px 10px 1px rgb(0 0 0 / 20%);font-family: helvetica, sans-serif;max-width: calc(100vw - 35px);overflow-y: auto;max-height: calc(100vh - 110px);">
    <div id="mod-loader-title" style="display: flex;justify-content: space-between;align-items: center;margin: 0px 0px 15px 0px;border: 2px solid var(--mod-loader-title-border);background-color: var(--mod-loader-title-bg);border-radius: 2px;">
      ...
      <span><a target="_blank" href="https://discord.gg/NA6vHg62An"><img title="Discord server" src="../../img/Discord.png" width="35px" style="margin-left: 3px; margin-right: 5px; position: relative; top: 2px;"></a></span>
    </div>
    ${gsmPromo}
  ...
  hideEndScreenImg.src = "../../img/EyeIcon.png";
